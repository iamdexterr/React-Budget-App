...
function BudgetCard({name,amount,max,gray,onAddExpenseClick,id,hideButtons,onViewExpensesClick}) {
  ...
  return (
    <Card className={classNames.join(' ')}>
        ...
        {max && <ProgressBar className='rounded-pill' variant={getProgressVariant(amount,max)} min={0} max={max}  now={amount}/>}
          {!hideButtons  && <Stack direction="horizontal" gap="2" className="mt-4">
                <Button variant='outline-primary' className='ms-auto' onClick={()=>{onAddExpenseClick(id)}}>Add Expense</Button>
